        ...
        <div className="mt-5" style={{flex: 1, textAlign: "center"}}>
            ...
            {user && (user._id === company.owner) ?
                <div className="company-update-btns" style={{margin: 'auto', width: '400px'}}>
                    ...
                    {isDeleteClicked ? <button className="company-button" onClick={deleteCompany}>I'm sure, DELETE</button> : <button className='company-button' onClick={() => setIsDeleteClicked(true)}>Delete Company</button>}
                </div>
                : null}
            <EditCompanyModal
